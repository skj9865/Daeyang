...
class ADC_GUI:
    ...
    def create_widgets(self):
        ...
        self.uart_combo = ttk.Combobox(self.root, values=self.uart_ports, state='readonly', width=12)
        ...
        # Refresh 버튼
        ttk.Button(self.root, text="Refresh", command=self.refresh_uart_ports).grid(row=0, column=2, padx=5)
        ...
        # 연결/해제 버튼
        ttk.Button(self.root, text="UART Connect", command=self.connect_uart).grid(row=0, columnspan=2,column=5, padx=15)
        ttk.Button(self.root, text="UART Disconnect", command=self.disconnect_uart).grid(row=0, columnspan=2,column=7, padx=15)
        ################### first line
        ...
        # ADC 채널 드롭다운        
        self.adc_channel_combo = ttk.Combobox(self.root, values=["ADC ch. 1", "ADC ch. 2", "ADC ch. 3"], state='readonly', width=12)
        self.adc_channel_combo.grid(row=1, column=8, padx=5)
        self.adc_channel_combo.current(0)  # 기본값 설정

        # ADC Setting 버튼
        ttk.Button(self.root, text="Reg setting", command=self.set_adc_channel).grid(row=2, column=8, pady=5)

        # Fault detection 및 모드 전환
        tk.Label(self.root, text="ADC fault detection mode setting").grid(row=3, column=0, columnspan=4, pady=5)
        self.mode_combo = ttk.Combobox(self.root, values=["Mode 1", "Mode 2", "Mode 3"], state="readonly", width=10)
        self.mode_combo.grid(row=3, column=4)
        self.mode_combo.current(0)  # 기본값 설정

        ttk.Button(self.root, text="Send", command=self.send_mode).grid(row=3, column=5, padx=5)
        ttk.Button(self.root, text="ADC Start", command=self.send_adc_start).grid(row=3, column=7, padx=5)
        ttk.Button(self.root, text="ADC Stop", command=self.send_adc_stop).grid(row=3, column=8, padx=5,pady=15)
        # Plot 설명
        tk.Label(self.root, text="Red : ADC1   Blue : ADC2   Green : ADC2").grid(row=4, column=0, columnspan=7, pady=5)
        ...
        self.status_label = tk.Label(self.root, text="Normal", bg="cornflower blue", width=20, height=10)
            ...
            if self.serial_port and self.serial_port.is_open:
                try:
                    ...
                    if head_val in [0x01, 0x02, 0x03]:  # ADC 데이터
                        ...
                        if len(payload) == 3:
                            adc_raw = (payload[0] << 16) | (payload[1] << 8) | payload[2]
